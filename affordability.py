...
class AffordabilityAnalyzer:
    ...
    def analyze_workforce(
        ...
        import logging
        import time
        logging.info("AFFORD ANALYZER: analyze_workforce() called")
        analyze_start = time.time()

        total_employees = len(census_df)
        logging.info(f"AFFORD ANALYZER: Total employees in census: {total_employees}")
        ...
        logging.info(f"AFFORD ANALYZER: Building employee locations for {len(employees_with_income)} employees with income...")
        ...
        logging.info(f"AFFORD ANALYZER: Built {len(employee_locations)} employee locations in {time.time() - locations_start:.2f}s")

        # Batch fetch LCSP for all unique combinations
        logging.info("AFFORD ANALYZER: Fetching LCSP batch...")
        lcsp_start = time.time()
        try:
            lcsp_df = PlanQueries.get_lcsp_for_employees_batch(db, employee_locations)
            logging.info(f"AFFORD ANALYZER: LCSP batch fetch completed in {time.time() - lcsp_start:.2f}s, got {len(lcsp_df)} rows")
        except Exception as e:
            logging.error(f"AFFORD ANALYZER: Error fetching LCSP data: {e}")
            return {
                'summary': {
                    'total_employees': total_employees,
                    'employees_analyzed': 0,
        ...
        logging.info("AFFORD ANALYZER: Building LCSP lookup dict...")
        lcsp_lookup = {}
        ...
        logging.info(f"AFFORD ANALYZER: Built lookup with {len(lcsp_lookup)} unique LCSP entries")

        # Calculate affordability for each employee
        logging.info("AFFORD ANALYZER: Calculating affordability for each employee...")
        calc_start = time.time()
        employee_details = []
        for idx, (_, employee) in enumerate(employees_with_income.iterrows()):
            if idx < 3 or idx % 20 == 0:
                logging.info(f"AFFORD ANALYZER: Processing employee {idx+1}/{len(employees_with_income)}")
        ...
        logging.info(f"AFFORD ANALYZER: Employee calculations complete in {time.time() - calc_start:.2f}s")

        # Aggregate statistics
        logging.info("AFFORD ANALYZER: Aggregating statistics...")
        ...
        logging.info(f"AFFORD ANALYZER: Analysis complete in {total_elapsed:.1f}s")
        logging.info(f"AFFORD ANALYZER: Processed {len(employee_details)} employees, {len(flagged_employees)} flagged")
...
class ContributionRecommender:
    ...
    # Fixed age tiers for age-based contribution strategy
    # These align with ICHRA/ACA rating patterns
    AGE_TIERS = [
    ...
    def generate_recommendations(
        ...
        Returns:
            ...
            [
                {
                    'strategy_type': 'age_banded' | 'location_based',
                    'name': str,
                    'tiers': List[dict],
                    'annual_cost': float,
        ...
        Strategy Logic:
            1. Age-banded (if CV > 0.15): Contiguous age bands based on LCSP variance
            2. Location-based (if multi-state & >10% variance): By state/rating area
        """
        ...
        # ====================
        # Strategy 1: Age-Banded (if variance exists)
        # ====================
        ...
        # ====================
        # Strategy 2: Location-Based (if multi-state with variance)
        # ====================
    ...
    def _generate_age_banded_recommendation(employees: List[dict]) -> Optional[dict]:
        """
        Generate age-banded contribution recommendation using fixed age tiers.

        Uses predefined age tiers that align with ICHRA/ACA rating patterns:
        - 21 (standalone)
        - 18-25
        - 26-35
        - 36-45
        ...
        Returns recommendation if there are employees with income data.
        """
        if not employees:
            return None

        # Get all employee ages
        employee_ages = [emp.get('age') for emp in employees if emp.get('age') is not None]
        if not employee_ages:
            return None

        # Build tiers from the fixed age tier definitions
        tiers = []
        for tier_def in ContributionRecommender.AGE_TIERS:
            age_min = tier_def['age_min']
            age_max = tier_def['age_max']
            age_range = tier_def['age_range']

            # Get employees in this age band
            # Note: Age 21 appears in BOTH the "21" standalone tier AND the "18-25" tier
            employees_in_band = [
                e for e in employees
                if e.get('age') is not None and age_min <= e.get('age') <= age_max
            ]

            # Calculate max contribution needed in this band (for affordability)
            # Filter out None values - only include employees with calculated min_er_contribution
            band_contributions = [
                e.get('min_er_contribution') for e in employees_in_band
                if e.get('min_er_contribution') is not None
            ]
            ...
            # Count employees with income data (those who have min_er_contribution calculated)
            employees_with_income = len([
                e for e in employees_in_band
                if e.get('min_er_contribution') is not None
            ])

            # Only include tiers that have employees or have a non-zero contribution
            # (to ensure coverage for all potential ages)
            tiers.append({
                'age_range': age_range,
                'contribution': tier_contribution,  # Keep full precision
                'count': len(employees_in_band),
                'count_with_income': employees_with_income,
                'ages': [age_min, age_max]
            })

        # Filter to only tiers with employees for cost calculation
        tiers_with_employees = [t for t in tiers if t['count'] > 0]

        if not tiers_with_employees:
            return None

        # Calculate annual cost (only for tiers with employees)
        annual_cost = sum(tier['contribution'] * tier['count'] * 12 for tier in tiers_with_employees)

        return {
            'strategy_type': 'age_banded',
            'name': 'Age-Based Contribution Tiers',
            'tiers': tiers,  # Include all tiers for display
            'tiers_with_employees': tiers_with_employees,  # For reference
...
class StrategyApplicator:
    ...
    def apply_strategy(
        ...
        Returns:
            New contribution_settings dict with structure:
            {
                ...
                'strategy_applied': 'age_banded' | 'location_based',
                'strategy_name': str,
                'classes': [...],
    ...
    def _apply_age_banded(
        ...
        """Apply age-banded contribution strategy using fixed age tiers."""
        ...
        # Use the fixed AGE_TIERS for matching, with contributions from strategy
        parsed_tiers = []
        tier_contrib_lookup = {t['age_range']: t.get('contribution', 0) for t in tiers}

        for tier_def in ContributionRecommender.AGE_TIERS:
            age_range = tier_def['age_range']
            contribution = tier_contrib_lookup.get(age_range, 0)
        ...
        # parsed_tiers is already sorted by age in AGE_TIERS definition
        ...
        for _, emp in census_df.iterrows():
            ...
            # Find matching tier
            # Priority: exact match (age 21 standalone) first, then range match
            matched_tier = None
            for tier in parsed_tiers:
                # For age 21, prefer the standalone "21" tier over the 18-25 range
                if tier['age_min'] == tier['age_max'] == 21 and age == 21:
                    matched_tier = tier
                    break
                # Skip the "21" standalone tier for other ages
                if tier['age_min'] == tier['age_max'] == 21:
                    continue
                # For other tiers, use standard range matching
                if tier['age_min'] <= age <= tier['age_max']:
                    matched_tier = tier
                    break

            # Fallback to 18-25 tier for ages under 18 (dependents)
            if matched_tier is None:
                for tier in parsed_tiers:
                    if tier['age_range'] == '18-25':
                        matched_tier = tier
                        break
                if matched_tier is None:
